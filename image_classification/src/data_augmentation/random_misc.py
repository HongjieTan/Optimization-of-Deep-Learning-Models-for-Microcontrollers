...
def random_periodic_resizing(
                images,
                resizing_period=None,
                random_image_sizes=None,
                interpolation=None,
                batch=None,
    ...
    """
    check_images_shape(images, function_name="random_periodic_resizing")
    check_dataaug_argument(resizing_period, "resizing_period", function_name="random_periodic_resizing", data_type=int, tuples=0)

    if batch > 0 and batch%resizing_period == 0:
        # Pick a new image size amongst the random sizes
        i = tf.random.uniform([1], minval=0, maxval=len(random_sizes), dtype=tf.int32, seed=seed)[0]
        target_size = (random_sizes[i], random_sizes[i])
    else:
        # Keep the same shape as at the the previous batch
        target_size = last_image_size
        
    # Resize if needed
    if target_size != images.shape[1:3]:
        images = tf.image.resize(images, (target_size[1], target_size[0]), method=method)
