...
def random_contrast(images, factor=None, pixels_range=(0.0, 1.0), change_rate=1.0):
    ...
    check_images_shape(images, function_name="random_contrast")
...
def random_brightness(images, factor=None, pixels_range=(0.0, 1.0), change_rate=1.0):
    ...
    check_images_shape(images, function_name="random_brightness")
    batch_size = tf.shape(images)[0]
...
def random_gamma(images, gamma=None, pixels_range=(0.0, 1.0), change_rate=1.0):
    ...
    check_dataaug_argument(gamma, "gamma", function_name="random_gamma_adjust", data_type=(int, float), tuples=2)
    if gamma[0] < 0 or gamma[1] < 0:
        raise ValueError("\nArgument `gamma` of function `random_gamma_adjust`: expecting "
                         "a tuple of 2 positive floats. Received {}".format(gamma))

    check_images_shape(images, function_name="random_gamma_adjust")
    batch_size = tf.shape(images)[0]
    width, height, channels = images.shape[1:]
...
def random_hue(images, delta=None, pixels_range=(0.0, 1.0), change_rate=1.0):
    ...
    check_images_shape(images, function_name="random_hue", allow_grayscale=False)
    batch_size = tf.shape(images)[0]
...
def random_saturation(images, delta=None, pixels_range=(0.0, 1.0), change_rate=1.0):
    ...
    check_images_shape(images, function_name="random_saturation", allow_grayscale=False)
    batch_size = tf.shape(images)[0]
    width, height = images.shape[1:3]
...
def random_value(images, delta=None, pixels_range=(0.0, 1.0), change_rate=1.0):
    ...
    check_images_shape(images, function_name="random_value", allow_grayscale=False)
    batch_size = tf.shape(images)[0]
    width, height = images.shape[1:3]
...
def random_hsv(images, 
    ...
    check_images_shape(images, function_name="random_hsv", allow_grayscale=False)
    batch_size = tf.shape(images)[0]
    width, height = images.shape[1:3]
...
def random_rgb_to_hsv(images, pixels_range=(0.0, 1.0), change_rate=0.25):
    ...
    check_images_shape(images, function_name="random_rgb_to_hsv", allow_grayscale=False)
    pixels_dtype = images.dtype
    images = remap_pixel_values_range(images, pixels_range, (0.0, 1.0), dtype=tf.float32)
...
def random_rgb_to_grayscale(images, pixels_range=(0.0, 1.0), change_rate=0.25):
    ...
    check_images_shape(images, function_name="random_rgb_to_grayscale", allow_grayscale=False)
    pixels_dtype = images.dtype    
    images = remap_pixel_values_range(images, pixels_range, (0.0, 1.0), dtype=tf.float32)
...
def random_sharpness(images, factor=None, pixels_range=(0.0, 1.0), change_rate=1.0):
    ...
    check_images_shape(images, function_name="random_sharpness")
    batch_size = tf.shape(images)[0]
    width, height, channels = images.shape[1:]
...
def random_posterize(images, bits=None, pixels_range=(0.0, 1.0), change_rate=1.0):
    ...
    check_images_shape(images, function_name="random_posterize")
    batch_size = tf.shape(images)[0]
...
def random_invert(images, pixels_range=(0.0, 1.0), change_rate=0.25):
    ...
    check_images_shape(images, function_name="random_invert")
...
def random_solarize(images, pixels_range=(0.0, 1.0), change_rate=0.25):
    ...
    check_images_shape(images, function_name="random_solarize")
    batch_size = tf.shape(images)[0]
...
def random_equalize(images, pixels_range=(0.0, 1.0), change_rate=0.25):
    """
    This function equalizes the histogram of images by spreading out
    the highly populated intensity values. It usually increases the 
    global contrast of images, especially when the image is represented
    by a narrow range of intensity values. It is useful in images with
    ...
    Returns:
        The equalized images. The data type and range of pixel values
        are the same as in the input images.
    """

    def equalize(image):
        histogram = tf.histogram_fixed_width(image, value_range=[0.0, 255.0], nbins=256)

        cdf = tf.math.cumsum(histogram)
        cdf = cdf / tf.math.reduce_sum(cdf)

        px_map = 255.0 * (cdf / tf.math.reduce_max(cdf))
        px_map = tf.cast(px_map, tf.int32)

        # Flatten the image
        imf = tf.reshape(image, [image.shape[0] * image.shape[1] * image.shape[2]])
        imf = tf.cast(imf, tf.int32)

        # Replace each pixel by its value in the histogram
        # Reshape the image to its original shape
        x = tf.gather(px_map, imf)
        x = tf.reshape(x, image.shape)
        eq_hist = tf.cast(x, tf.float32)

        return eq_hist

    check_images_shape(images, function_name="random_equalize")
    pixels_dtype = images.dtype
    images = remap_pixel_values_range(images, pixels_range, (0.0, 255.0), dtype=tf.float32)
    ...
    padding = images_aug[1:]

    for i in range(batch_size):
        equalized = equalize(images[i])
        # Add the equalized image to the batch
        x = tf.expand_dims(equalized, axis=0)
        padded = tf.concat([x, padding], axis=0)
        images_aug += tf.roll(padded, shift=i, axis=0)
...
def random_autocontrast(images, cutoff=10, pixels_range=(0.0, 1.0), change_rate=0.25):    
    ...
    if cutoff < 0 or cutoff > 100:
        raise ValueError("Argument `cutoff` of function `random_autocontrast`: value must be an integer "
                         "in the interval [0, 100]. Received {}".format(cutoff))

    check_images_shape(images, function_name="random_autocontrast")
    ...
    def scale_channel(image):
        ...
        w, h = image.shape[1:]      
        lower_cut = upper_cut = int((w * h * cutoff) // 100)
    ...
    if images.shape[-1] == 3:
        ...
        # Grayscale images
        images_aug = scale_channel(images[..., 0])
        images_aug = tf.expand_dims(images_aug, axis=-1)
