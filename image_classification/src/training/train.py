...
def load_model_to_train(cfg, model_path=None, num_classes=None) -> tf.keras.Model:
    ...
    if cfg.model:
        ...
        keras.utils.plot_model(model, to_file=HydraConfig.get().runtime.output_dir+"\\"+model.name+".png", show_shapes=True)
        visualkeras.layered_view(model, legend=True).save(HydraConfig.get().runtime.output_dir+"\\layers.png")
