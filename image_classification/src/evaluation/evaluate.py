...
def evaluate_tflite_quantized_model(quantized_model_path: str = None, eval_ds: tf.data.Dataset = None,
    ...
    f1 = sklearn.metrics.f1_score(labels, logits,average='macro')
    precision = sklearn.metrics.precision_score(labels, logits,average='macro')
    recall = sklearn.metrics.recall_score(labels, logits,average='macro')
    accuracy = round((np.sum(labels == logits) * 100) / len(test_labels), 2)
    print("[INFO] : Accuracy of quantized model = {} %".format(accuracy))
    print("[INFO] : f1 of quantized model = {} ".format(f1))
    print("[INFO] : precision of quantized model = {} ".format(precision))
    print("[INFO] : recall of quantized model = {} ".format(recall))
    sklearn.metrics.classification_report(labels, logits)
    log_to_file(output_dir,  "\n" + "Quantized model :")
    log_to_file(output_dir, f"Accuracy of quantized model : {accuracy} %")
    log_to_file(output_dir, f"f1 of quantized model : {f1} ")
    log_to_file(output_dir, f"precision of quantized model : {precision} ")
    log_to_file(output_dir, f"recall of quantized model : {recall} ")
