...
def _stm32_get_info(app, serial_number=None):
...
def get_stm32_board_interfaces() -> Tuple[List[dict], List[dict]]:
    ...
    if app:
        ...
        for idx, st_link in enumerate(st_links):
            st_links[idx] = _stm32_get_info(app[0], serial_number=st_link['sn'])
